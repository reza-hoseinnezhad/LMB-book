function tt_lmb= glmb2lmb(glmb)
    ...
    for hidx=1:length(glmb.w) %each hypothesis
       for t= 1:glmb.n(hidx) %each element in a hypothesis
          ...
          tt_lmb{newidx}.w(Particle_Index(newidx):(Particle_Index(newidx)+length(glmb.tt{trkidx}.w)-1)) = glmb.w(hidx)*glmb.tt{trkidx}.w;
    ...
    for tabidx=1:length(tt_lmb)
       tt_lmb{tabidx}.r= sum(tt_lmb{tabidx}.w); % ensures no potential matlab rounding error to 1
