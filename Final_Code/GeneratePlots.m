function GeneratePlots(Truth,Meas,Estimates)
    ...
    for i = 1:Truth.total_tracks
        ...
        h = quiver(Tracks{i}(1,1),Tracks{i}(2,1),10*Tracks{i}(3,1),10*Tracks{i}(4,1),'k',...
            'LineWidth',4,'MaxHeadSize',0.8);
    ...
    for k = 1:K
        Z = Meas.Z{k};
        Theta = Z(1,:); Rho = Z(2,:);
        plot(Rho.*sin(Theta),Rho.*cos(Theta),'b.');
    end
    ...
    for k = 1:K
        if ~isempty(Estimates.L{k})
            for j = 1:size(Estimates.L{k},2)
                ...
                plot(Estimates.X{k}(1,j),Estimates.X{k}(2,j),'o',...
                    'MarkerEdgeColor',DotColor,'MarkerFaceColor', DotColor, ...
                    'MarkerSize', 4);
            end
        end
    end
