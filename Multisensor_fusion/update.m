function glmb_update = update(glmb_predict, model, filter, Z, sen_loc, k)
    ...
    for emm = 1:m
        for tabidx = 1:length(glmb_predict.tt)
            ...
            w_temp = compute_pD(glmb_predict.tt{tabidx}.x,sen_loc,model) .* glmb_predict.tt{tabidx}.w(:) .*compute_likelihood(model, Z{k}(:, emm), sen_loc,glmb_predict.tt{tabidx}.x);
    ...
    for tabidx = 1:length(glmb_predict.tt)
        if ~isempty(glmb_predict.tt{tabidx}.w)
            avpd(tabidx) = glmb_predict.tt{tabidx}.w(:)'*compute_pD(glmb_predict.tt{tabidx}.x,sen_loc,model) + eps(0);
        end
    end
    avqd = 1 - avpd;
    ...
    if m == 0
        ...
        for pidx = 1:length(glmb_predict.w)
            glmb_update.w(pidx) = -model.lambda_c + sum(log(avqd(glmb_predict.I{pidx}))) + log(glmb_predict.w(pidx)); 
        ...
        for pidx = 1:length(glmb_predict.w)
            if glmb_predict.n(pidx) == 0
                ...
                costm = allcostm(glmb_predict.I{pidx}, :) ./ (model.lambda_c * model.pdf_c * repmat(avqd(glmb_predict.I{pidx}), [1 m])); 
                % Cost matrix
                neglogcostm = -log(costm); % Negative log cost
                % Murty's algorithm to calculate m-best 
                % assignment hypotheses/components
                [uasses, nlcost] = mbestwrap_updt_custom(neglogcostm, round(filter.H_upd * sqrt(glmb_predict.w(pidx)) / sum(sqrt(glmb_predict.w)))); 
                % Generate corresponding surviving hypotheses
                % or components
                for hidx = 1:length(nlcost)
                    update_hypcmp_tmp = uasses(hidx, :)';
                    % Hypothesis/component weight
                    glmb_update.w(runidx) = -model.lambda_c + m * log(model.lambda_c * model.pdf_c) + sum(log(avqd(glmb_predict.I{pidx}))) + log(glmb_predict.w(pidx)) - nlcost(hidx);
